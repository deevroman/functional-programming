...
balance_check({nil, nil, H, nil, nil}) -> H == 0;
balance_check({_, _, H, S, B}) ->
  H1 = ?HEIGHT(S),
  H2 = ?HEIGHT(B),
  balance_check(S) and balance_check(B)
    and (-2 < H2 - H1) and (H2 - H1 < 2)
    and (H == max(H1, H2) + 1).
...
simple_node_remove_test() ->
  ...
  ?assertEqual(avldict:to_list(avldict:remove(1, T)), [{0, 0}]).

simple_node2_remove_test() ->
  T = avldict:from_list([{1, 1}, {0, 0}, {2, 2}]),
  ?assertEqual(avldict:to_list(avldict:remove(2, T)), [{0, 0}, {1, 1}]),
  ?assertEqual(avldict:to_list(avldict:remove(1, T)), [{0, 0}, {2, 2}]).

simple_node4_remove_test() ->
  T = avldict:from_list([{1, 1}, {0, 0}, {2, 2}, {4, 4}]),
  ?assertEqual(avldict:to_list(avldict:remove(2, T)), [{0, 0}, {1, 1}, {4, 4}]).

simple_not_found_remove_test() ->
  L = [{I, I} || I <- lists:seq(0, 3)],
  T = avldict:from_list(L),
  ?assertEqual(avldict:to_list(avldict:remove(42, T)), L).

remove_all(L, T) ->
  {R, _} = lists:foldl(fun({K, _V}, {TAcc, LAcc}) ->
...
monoid_assoc_test_case(ASize, BSize, CSize) ->
  ...
  B = avldict:from_list([{random:uniform(30), random:uniform(100)} || _ <- lists:seq(1, BSize)]),
  C = avldict:from_list([{random:uniform(30), random:uniform(100)} || _ <- lists:seq(1, CSize)]),

  R1 = avldict:merge(avldict:merge(A, B), C),
  R2 = avldict:merge(A, avldict:merge(B, C)),
  ?assertEqual(avldict:to_list(R1), avldict:to_list(R2)).

monoid_assoc_small_test() ->
  [monoid_assoc_test_case(random:uniform(10) - 1, random:uniform(10) - 1, random:uniform(10) - 1)
    || _ <- lists:seq(1, 10000)].

monoid_assoc_medium_test() ->
  [monoid_assoc_test_case(random:uniform(50) - 1, random:uniform(50) - 1, random:uniform(50) - 1)
    || _ <- lists:seq(1, 10000)].

monoid_assoc_big_test() ->
  [monoid_assoc_test_case(random:uniform(100) - 1, random:uniform(100) - 1, random:uniform(100) - 1)
    || _ <- lists:seq(1, 10000)].
